...
; Main Program
;
pro spectra_tim_Zps4,cm_eph,cm_val,state,Dat5,BB,noi
;
;Common blocks
;
...
Ttle[0]='Orbit'+orb+' '+orb_date+' BXY Power Spectrum'
Ttle[1]='Orbit'+orb+' '+orb_date+' SZ Poynting Spectrum'
tempttle=ttle
;stop
...
DispArr[*,*,0]=CPArr
DispArr[*,*,1]=PoyntArr[*,*,2]
mxpow_z=dblarr(2)
mnpow_z=dblarr(2)
mxpow_z[0]=max(cparr)
mnpow_z[0]=min(cparr)
mxpow_z[1]=max(Poyntarr[*,*,2])
mnpow_z[1]=min(Poyntarr[*,*,2])
if abs(Mxpow_z[1]) GE abs(Mnpow_z[1]) then $
Mnpow_z[1]=-Mxpow_z[1] else $
Mxpow_z[1]=abs(Mnpow_z[1])

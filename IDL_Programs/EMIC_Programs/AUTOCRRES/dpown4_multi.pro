...
; Main Program
;
Pro DPOWN4_multi,cm_eph,cm_val,state,Dat5,noi
...
Widget_control,state.dat_info,$
 set_value=string('Calculating power spectra with default values......')
 Widget_control,state.text,$
set_value=string('Calculating power spectra with default values......'),/append,/show
...
NyqF=1.0/(2.0*SInt)
Widget_control,state.text,$
set_value='The Nyquist is '+string(NyqF)+' Hz',/append,/show
...
MxFr=(NFr-1)*DelF
Widget_control,state.text,$
set_value='The Frequency Resolution is '+string(DelF)+'Hz',/append,/show
...
Begin
 ...
 Posn=long((long(Bloc)+1)*long(TRes))
 For j=long(0),long(FFTN)-1 do TsArr(long(j))=XDat(long(Posn)+long(j),long(q)) ; New Data
End    ; end Bloc Loop
;
endfor
...
Widget_control,state.text,$
set_value='Maximum Power is '+string(MxRng[q])+'dB in '+string(datt5[q]),/append,/show
Widget_control,state.text,$
set_value='Minimum Power  is '+string(MnRng[q])+'dB in '+string(datt5[q]),/append,/show
end

if MxRng[0] GE MxRng[1] and MxRng[0] GE MxRng[2] then $
new_MxRng=MxRng[0] else $

if MxRng[1] GE MxRng[0] and MxRng[1] GE MxRng[2] then $
new_MxRng=MxRng[1] else $

new_MxRng=MxRng[2]


if MnRng[0] GE MnRng[1] and MnRng[0] GE MnRng[2] then $
new_MnRng=MnRng[0] else $

if MnRng[1] GE MnRng[0] and MnRng[1] GE MnRng[2] then $
new_MnRng=MnRng[1] else $

new_MnRng=MnRng[2]

print,'new_maxrng',new_MxRng
print,'new_minrng',new_MnRng
Widget_control,state.text,$
set_value='Setting Minimum Power  to '+string(new_MnRng)+'dB',/append,/show
Widget_control,state.text,$
set_value='Setting Maximum Power  to '+string(new_MxRng)+'dB',/append,/show
for q=0,2 do $
	begin
 PAgn=0
 ...
 Scale=Scl,Range=[new_MnRng,new_MxRng],Aspect=5.0,dat5=datt5[q],$

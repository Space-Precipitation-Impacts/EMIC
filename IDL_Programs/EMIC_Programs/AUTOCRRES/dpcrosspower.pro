...
; Main Program
;
Pro dpcrosspower,cm_eph,cm_val,state,Dat5,noi
...
CPArr=10*Alog(CPArr)
;*********************************************************************************
;Threshold for power set at -20dB. 14/09/01 Tapu'osi Loto'aniu
;
;
;index=where(cparr LT -20, count)
;cparr[index]=-20.0
;
;*********************************************************************************
...
;REPEAT Begin
 ...
 Widget_control,state.text,$
set_value='Maximum Cross Power is '+string(Max(CPArr))+'dB',/append,/show
Widget_control,state.text,$
set_value='Minimum Cross Power is '+string(Min(CPArr))+'dB',/append,/show
...
;spectralps,cm_eph,cm_val,state,Dat5,CPArr,Fname
;DPChoice3,Dat5
;stop

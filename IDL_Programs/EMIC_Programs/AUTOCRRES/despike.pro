...
for yy=0,1 do $
begin
index_max=where(max(onee) EQ onee ,count_max)
if count_max GT 0 then $
	 for gg=0,count_max-1 do $
   		onee[index_max[gg]]=median(one)

index_min=where(min(onee) EQ onee ,count_min)
if count_min GT 0 then $
	 for gg=0,count_min-1 do $
   		onee[index_min[gg]]=median(one)
end

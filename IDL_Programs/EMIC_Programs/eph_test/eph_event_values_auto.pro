...
pro eph_EVENT_VALUES_auto,cm_eph,cm_val,state,noi
...
if fix(para_struct(noi).orbit) LT 1000 then $
	begin
;********************************************************************
...
;********************************************************************
	...
	begin
	;********************************************************************
stime=double(strmid(para_struct(noi).start_time,0,3))*60.*60.*1000.+ double(strmid(para_struct(noi).start_time,3,2))*60.*1000.

etime=double(strmid(para_struct(noi).end_time,0,3))*60.*60.*1000.+ double(strmid(para_struct(noi).end_time,3,2))*60.*1000.
endelse
;
;stop
...
eph_names = strarr(n_elements(nname)-2)
eph_data_array = fltarr(n_elements(nname)-2,ww)
for j=0,n_elements(nname)-3 do $
begin
eph_names[j]=nname[j+2]
;for ja=0,n_elements(ww)-1 do $
eph_data_array[j,*]=cm_eph.(j+2)[index]
end
julian_day = cm_eph.(0)
u_time_formated = utarr
u_time_milsec = utt
eph_EVENT_VALUES_cal_auto,noi
;stop

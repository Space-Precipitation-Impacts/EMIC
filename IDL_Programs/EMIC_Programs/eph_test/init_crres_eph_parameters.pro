...
;wait,0.5
for kk=0,no_data-1 do $
begin
WIDGET_CONTROL, state3.orb_info, set_value=string(data_files[kk])
;wait,0.5
end
...
;stop
if fix(strcompress(strmid(text,0,4))) LT 1000 then $
begin
	         para_struct[j].(0) = strcompress('Orb'+strmid(text,0,4)+'_'+strmid(strcompress(text),4,4)+'_'+strmid(strcompress(text),9,4)+'a.val')
	         para_struct[j].(1) = strmid(text,0,4)
	         para_struct[j].(2) =strmid(strcompress(text),4,4)
	         para_struct[j].(3) =strmid(strcompress(text),9,4)
  ...
  begin
  	         para_struct[j].(0) = strcompress('Orb'+strmid(text,0,4)+'_'+strmid(strcompress(text),4,5)+'_'+strmid(strcompress(text),9,5)+'a.val')
	         para_struct[j].(1) = strmid(text,0,4)
	         para_struct[j].(2) =strmid(strcompress(text),4,5)
	         para_struct[j].(3) =strmid(strcompress(text),9,5)
	         para_struct[j].(4) =FLOAT(strmid(text,15,3))
	         para_struct[j].(5) =FLOAT(strmid(text,15,3))+1.0

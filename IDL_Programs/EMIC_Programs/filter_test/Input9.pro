...
;
;
Pro Rev, XDat,Ydat, LenX
  For i=0, LenX-1 do $
   begin
     Ydat[i] = Ydat[LenX - i -1]
     XDat[i] = Xdat[LenX - i -1]
   endfor
end
...
Set_plot,'WIN'
!P.MULTI = [0, 1, 2]
Window,0, Xsize=600,Ysize=400,title='B wave field before reverse filtering and low pass filtering'
...
Theta=fltArr(6)
FiltType=2
FiltCtrl,Bx,Bxtime/1000.0,wc,Order,FiltType,Cut,countBx,theta
;FiltCtrl,By,Bytime/1000.0,wc,Order,FiltType,Cut,countBy,theta
;FiltCtrl,Bz,Bztime/1000.0,wc,Order,FiltType,Cut,countBz,theta
;FiltCtrl,Bxx,Bxxtime/1000.0,wc,Order,FiltType,Cut,countBxx,theta
;FiltCtrl,Byy,Byytime/1000.0,wc,Order,FiltType,Cut,countByy,theta
;FiltCtrl,Bzz,Bzztime/1000.0,wc,Order,FiltType,Cut,countBzz,theta
plot,Bxtime,Bx,title='Bx (wave field) filtered with phase shift',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
...
print,'About to run B field data through reverse 2nd order Butterworth filter at 6.0Hz'
wait,1.0
Rev,Bx,Bxtime/1000.0,countBx
Rev,By,Bytime/1000.0,countBy
Rev,Bz,Bztime/1000.0,countBz
Rev,Bxx,Bxxtime/1000.0,countBxx
Rev,Byy,Byytime/1000.0,countByy
Rev,Bzz,Bzztime/1000.0,countBzz
Print,'Data reversed now running through Butterworth filter.........'
wait,1.0
Order = 2
cut = float(6.0/Bsfreq)
Theta=fltArr(6)
...
FiltCtrl,Byy,Byytime/1000.0,wc,Order,FiltType,Cut,countByy,theta
FiltCtrl,Bzz,Bzztime/1000.0,wc,Order,FiltType,Cut,countBzz,theta
Print,'Data phase shift removed......'
wait,0.5
Print,'Data unreversing..........'
wait,0.5
Rev,Bx,Bxtime/1000.0,countBx
Rev,By,Bytime/1000.0,countBy
Rev,Bz,Bztime/1000.0,countBz
...
Rev,Byy,Byytime/1000.0,countByy
Rev,Bzz,Bzztime/1000.0,countBzz
plot,Bxtime,Bx,title='Bx (wave field) with phase shift removed',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
wait,1.0
stop
...
if (ans = 1) then $
 ...
 ;Theta=fltArr(6)
  ...
  Wait,0.5
  Order = 2
  FiltCtrl,Ey,Eytime/1000.0,wc,Order,FiltType,Cut,countE12,theta
  FiltCtrl,Ez,Bztime/1000.0,wc,Order,FiltType,Cut,countE34,theta
  !P.Multi = [0,1,2]
  window,1,xsize=400,ysize=400, title ='E wave fields low pass filtered'
  ...
  FiltCtrl,Bz,Bztime/1000.0,wc,Order,FiltType,Cut,countBz,theta
  !P.MULTI = [0,1,3]
  window,2,xsize=400,ysize=400, title='B wave field after low pass filtering'
  ...
  Wait,0.5
  Order = 2
  FiltCtrl,Bxxtime/1000.0,Bxx,wc,Order,FiltType,CutFreq,countBxx,theta
  FiltCtrl,Byytime/1000.0,Byy,wc,Order,FiltType,CutFreq,countByy,theta
  FiltCtrl,Bzztime/1000.0,Bzz,wc,Order,FiltType,CutFreq,countBzz,theta
  window,0,xsize=400,ysize=400, title='B background plus perturbed after lowpass filtering'
  ...
  If countE12 LE countBx then $
    ...
    begin
     num = countBx
  endelse
     Ey = Congrid(Ey,fix(num/2.0))
     Ez = Congrid(Ez,fix(num/2.0))
     Eytime = Congrid(Eytime,fix(num/2.0))
     Bx = Congrid(Bx,fix(num/2.0))
     By = Congrid(By,fix(num/2.0))
     Bz = Congrid(Bz,fix(num/2.0))
     Bxtime = Congrid(Bxtime,fix(num/2.0))
     Bxx = Congrid(Bxx,fix(num/2.0))
     Bxxtime = Congrid(Bxxtime,fix(num/2.0))
     Byy = Congrid(Byy,fix(num/2.0))
     Bzz = Congrid(Bzz,fix(num/2.0))
     array = fix(num/2.0)
     Esample = abs(Eytime[1] - Eytime[0])     ;E field sampling time
     Esfreq = round(1000.0/Esample)           ;E field sampling frequency
     Bsample = abs(Bxtime[1] - Bxtime[0])     ;B field sampling time
     ...
     Print,'The new sampling frequency is',Bsfreq,'Hz'
  window,0,xsize=600,ysize=400, title='B wave field after low pass filtering and resampling'
  plot,Bxtime,By,title='Bx (wave field)',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
  plot,Bxxtime,Byy,title='By (main field)',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
  plot,Eytime,Ez,title='Ez (wave field)',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT
  endif else $
    begin
      Print,'Recommend no resampling due to possible aliasing above Nyquist'
      wait,1.5
      If (countE12 LE countBx) and (countE12 LE countBxx) then $
         array = countE12
      If (countBx LT countE12) and (countBx LT countBxx) then $
         array = countBx
      If (countBxx LT countBx) and (countBxx LT countE12) then $
         array = countBxx
  endelse

                                         ; Used for BandPass

                                         ;Calling procedure to determine if Ex
...
for mm=0,array -1 do $                                          ;Begin loop to determine alpha angle.
 begin                  ;Note: all perturb B fields have same array size so
...
COEFFx = POLY_FIT(Bxxt,Bxxs,1)
COEFFy = POLY_FIT(Byyt,Byys,1)
COEFFz = POLY_FIT(Bzzt,Bzzs,1)
BXMAIN = POLY(Bxxt,COEFFx)
BYMAIN = POLY(Bxxt,COEFFy)
BZMAIN = POLY(Bxxt,COEFFz)
dBx = Bxxs - Bxmain
dBy = Byys - Bymain
dBz = Bzzs - Bzmain
;
...
Sz2 = Sx1*cos(theta)+Sz1*sin(theta)
;
...
for t=0,nn-1 do $
 begin
 angleSB[t] = ThetaSB(Sx3[t],Sy3[t],Sz3[t])
endfor
  window,0,xsize=400,ysize=400,title='Sz vector before bandpass filtering'
  plot,Bzt,Sz3,title='Sz',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
  Print,'The Poynting vector power spectrum may include other pulsation activity.'
  Print,'To remove this affect we must bandpass filter Poynting vector results.'
  ;Read,FiltType
  Order = 2
  Print,'Enter the LOW Frequency cutoff (Hz) : '
  Read,cutfreq
  ;CutFreq=CutFreq/1000.0
  CutFreq=float(CutFreq/Bsfreq)
  FiltType = 1
  Print,'High passing the data above',CutFreq,'Hz'
  FiltCtrl,Bzt,Sz3,wc,Order,FiltType,CutFreq,nn,theta
  FiltFlag=FiltType
  print,'Enter the HIGH Frequency cut off (Hz) :'
  Read,CutFreq
  CutFreq=float(CutFreq/BsFreq)
  FiltType=2
  Print,'Low Pass Filtering Data below',CutFreq,'Hz'
  Wait,0.5
  FiltCtrl,Bzt,Sz3,wc,Order,FiltType,CutFreq,nn,theta
 ;FiltType=FiltFlag
  window,1,xsize=400,ysize=400,title='Sz vector after bandpass filtering'
  plot,Bzt,Sz3,title='Sz',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
  stop
 !P.MULTI = [0, 1, 3]                     ;
 set_plot,'WIN'
 ...
 plot,Bzt,Sz3,title='Sz',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
 plot,Bzt,angleSB,title='angleSB',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
 plot,Bzt,Ex3,title='Ex',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'

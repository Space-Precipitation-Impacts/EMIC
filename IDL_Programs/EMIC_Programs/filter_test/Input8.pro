...
; If (k mod 15000) eq 0 then print,'running smoothly after:',k,' data points'
; endwhile
 ;stop
 While (k LT count_rows) do $            ;Loop to begin data input
 begin
...
alpha_angle = fltarr(countBx)            ;Initialize alpha angle handling array.


n=long(0)
for m=0,countBx -1 do $                  ;Note: all perturb B fields have same array size so
...
B = fltarr(nn)
nnn = Long(0)
for mm=long(0),countE12 -1 do $;Begin loop to determine alpha angle.
...
;Polynomial fit for main B field
;COEFFx = FLTARR(NN)
;COEFFy = FLTARR(NN)
;COEFFz = FLTARR(NN)
;BXMAIN = FLTARR(NN)
...
plot,Bzt,angleSB*(1.0/pI)*180,title='angleSB',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'

...
Pro input14                             ;Start of main body
pI=3.1415926535898
...
While (NOT EOF(units)) DO $            ;Loop to count total data rows and
 BEGIN                                 ;data component rows in file.
  ...
  If index eq Indx[0] then countEy=countEy+long(1) ;By(nT) unfiltered counter
  If index eq Indx[1] then countEz=countEz+long(1) ;Bz(nT) unfiltered counter
...
COEFFx = POLY_FIT(ttn,Bxxn,1)
COEFFy = POLY_FIT(ttn,Byyn,1)
COEFFz = POLY_FIT(ttn,Bzzn,1)
BXMAIN = POLY(ttn,COEFFx)
BYMAIN = POLY(ttn,COEFFy)
BZMAIN = POLY(ttn,COEFFz)
;
;Rotation routine
;
theta = fltarr(n)
...
for q=0,n-1 do $
 ...
 theta[q] = atan(sqrt(BXmain[q]*Bxmain[q]+Bymain[q]*Bymain[q])/abs(Bzmain[q]))
  if (BXMAIN[q] LT 0.0) then theta[q] = pI-theta[q]
  phi[q] = atan(abs(BYMAIN[q]/BXMAIN[q]))
  if(BYMAIN[q] LT 0.0) AND (BXMAIN[q] LT 0.0) then phi[q] = phi[q] + pI
  if(BYMAIN[q] LT 0.0) and (BXMAIN[q] GT 0.0)then phi[q] = -phi[q]
  if(BXMAIN[q] LT 0.0) and (BYMAIN[q] GT 0.0)then phi[q] = pI-phi[q]
endfor
;
...
By1 = Byn*cos(phi)-Bxn*sin(phi)
Bx1 = Bxn*cos(phi)+Byn*sin(phi)
Ey1 = Eyn*cos(phi) - Exn*sin(phi)
Ex1 = Exn*cos(phi)+Eyn*sin(phi)
...
set_plot,'ps'
device, filename="c:\Paul\phd\crres\orb86Sz.ps", yoffset=3, ysize=25
!P.MULTI = [0, 1, 6]                     ;
;window,0,xsize=700,ysize=500
plot,ttn,Sz2,title='Sz'+header[0],XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,angleSsz/Pi*180,title='angleSB',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,Ex2,title='Ex',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,Ey2,title='Ey',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,By2,title='By',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,Bx2,title='Bx',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
device,/close
Cut =0.0
theta = fltarr(6)
...
plot,ttn,S,title='S',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
set_plot,'ps'
device, filename="c:\Paul\phd\crres\orb86Sz2.ps", yoffset=3, ysize=25
!P.MULTI = [0, 1, 5]                     ;
plot,ttn,Sz2,title='Sz'+header[0],XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,angleSsz/Pi*180,title='angleSB',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'
plot,ttn,Ex2,title='Ex',XStyle=1,Xticks =3,XTickFormat='XTLab',xtitle ='Time (UT)'

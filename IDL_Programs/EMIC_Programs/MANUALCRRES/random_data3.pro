...
for i=0,ct-1 do $
          begin
             ...
             data_32[i] = 2.0*sin(2*pi*30*i)+2.0*sin(2*pi*30*i+pi)+2.0*sin(2*pi*30*i)
		;		frq[i] = 0.5/1000.*i
endfor
count=long(0)
datafft=(abs(FFT(data_32,-1)))^2.0
N21 = 1000./2 + 1	;Midpoint+1 is the most negative frequency subscript.
;F = INDGEN(N)	;The array of subscripts.
for i=0,500-2 do $
datafft[N21+i] = 0.0 ;datafft[N21-i-1];Insert negative frequencies in elements F(N/2 +1), ..., F(N-1).
;F = F/(N*T)

!P.Multi = [0,1,2]
set_plot,'WIN'
Window,1, Xsize=750,Ysize=700,title='Wave fields'
  ...
  ;  plot,frq,shift(datafft,250),xtitle ='Frequency (Hz)',ymargin=[5,5],xmargin=[10,12];,$
print,max(datafft)
print,min(datafft)
Xdat=data_32
;stop
...
Begin
 ...
 TrArr=FFT(TsArr,1)    ;Now implemented with reverse FFT
 ;stop
 For i=0,NFr-1 do $
 Begin
 ;stop
...
;MxxRng=MxRng
;MnnRng=MnRng
;DDisparr=Disparr*V2*V2
;***********************************************************************
;calculate multiplication factor
;
fac=(max(Xdat)^2.0)/max(Disparr)
;print,fac
;fac=7.1
...
;
;Disparr = 10*Alog10(Disparr/MxxRng)
;**********************************************************************
;Restrict Power dB to 0 -> -5
;
for i=0,n_elements(Disparr)-1 do $
...
;REPEAT Begin
 ...
 XTtle='Time (UT)'
 YTtle='Frequency (Hz)'
CbTtle='Power (nT!U2!n/Hz)'
 YRngL=0
 Scl=1
 ;!P.multi=0
 dat5='test'
 DYNTV_crres_test,DispArr,Cbtle=Cbttle,Title=Ttle,YTitle=YTtle,xtitle='UT',$
 XRange=[Min(tims_32[0]),Max(tims_32[Npnts-1])],YRange=[YRngL,MxFr],$
 Scale=Scl,Range=[MnRng,MxRng],Aspect=3.0,ymargin=[10,-5],xmargin=[10,10],dat5=dat5
;endelse
PntRes=TRes
MnPow=MnRng
MxPow=MxRng
 Print,'Maximum Power in Decibels ',string(Max(DispArr),Format='(F6.3)'),'dB'
 Print,'Minimum Power in Decibels ',string(Min(DispArr),Format='(F6.3)'),'dB'

print,'IDL FFT routine only max ', string(max(datafft),Format='(F6.3)')
print,'IDL FFT routine only min',string(min(datafft),Format='(F6.3)')
;print,'IDL FFT routine using Colins code without Dtrend max ', string(MxxRng*V2^2,Format='(F6.3)')
;print,'IDL FFT routine using Colins code without Dtrend min',string(MnnRng*V2^2,Format='(F6.3)')

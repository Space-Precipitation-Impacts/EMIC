...
;BZOLD=BZmain
;dtrend2,Bzmain,count0,Raz,Rbz
; Bzzz=Bz[count0-1]
;BZNEW=BZ
;stop
;BXOLD=BXmain
; dtrend2,Bxmain,count0,Rax,Rbx

 ;STOP
;Byold=Bymain
; dtrend2,Bymain,count0,Ray,Rby
;STOP
; Byyy=By[count0-1]
...
Pc5y1 = Pc5y*cos(phi)-Pc5x*sin(phi)
EPc5y1 = EPc5y*cos(phi)-EPc5x*sin(phi)

BXmain1 = BXmain*cos(phi)+BYmain*sin(phi)
dBx1 = dBx*cos(phi)+dBy*sin(phi)
Pc5x1 = Pc5x*cos(phi)+Pc5y*sin(phi)
EPc5x1 = EPc5x*cos(phi)+EPc5y*sin(phi)
...
Pc5x2 = Pc5x1*cos(theta)-Pc5z1*sin(theta)
EPc5x2 = EPc5x1*cos(theta)-EPc5z1*sin(theta)
Ex2 = Ex1*cos(theta)-Ez1*sin(theta)
Sx2 = Sx1*cos(theta)-Sz1*sin(theta)

dBz2 = dBz1*cos(theta)+dBx1*sin(theta)
Pc5z2 = Pc5z1*cos(theta)+Pc5x1*sin(theta)
EPc5z2 = EPc5z1*cos(theta)+EPc5x1*sin(theta)
...
Pc5y3 = Pc5y2*cos(-phi)-Pc5x2*sin(-phi)
EPc5y3 = EPc5y2*cos(-phi)-EPc5x2*sin(-phi)
...
Pc5x3 = Pc5x2*cos(-phi)+Pc5y2*sin(-phi)
EPc5x3 = EPc5x*cos(-phi)+EPc5y2*sin(-phi)

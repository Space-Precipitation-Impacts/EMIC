...
;**************************************************************************
 if ev.id EQ state.fil_menu then $
	begin
 ;case switch for parent widget main menu option "File".
 ;Note: Implemented here in if blocks but should be changed to case blocks.
 ;*************************************************************************
    	if ev.value EQ 'New' then $
     		begin
 				;Initializing state variables
	  			...
	  			WIDGET_CONTROL, state.ser_menu, sensitive=0
 ...
 ;**************************************************************************
 ;File->"Open" option used to select ephmerius and/or data files for input
 if ev.value EQ 'Open'then $
     begin
 ;**************************************************************************
 ;File selection is implemented as a Pop-Up file selection dailog widget
 ;No filter on file type as *.0ep or *.val maybe selected.
 ;en->holds complete PATH to file
 ;enn->holds file name
		...
		enn=strlen(Fname)
 ;**************************************************************************
 ;Option for only *.0ep selected
 ;countt=0
 ;index=where(path EQ '.',countt)
 ;stop
       if strlowcase(strmid(Fname,enn-3,3)) EQ '0ep' then $
         begin
          orb_binfile=strmid(Fname,en,8)
 ;Checks crfiles.shr file for obrit number and date.
 		  ;cd,strmid(path,0,strlen(path)-5)
          eph_crfiles,orb,orb_binfile,orb_date
          ;stop

 ;Opens ephemeruis file and inputs parameters values
       	  fileph,FName,ephv,path,state
 ;Pass parameter values to ephemerius data strucuture cm_eph.
        cm_eph=ephv
 ;Intialize required parent widget menus to handle ephemerius
        ...
        SET_VALUE ='View ephmerius parameters using Ephmerius menu'
 ...
 ;Option if telemetry *.val file selected
      if strmid(Fname,enn-3,3) EQ 'val' then $
        begin
		orbtemp=fix(strcompress(strmid(Fname,en+3,4)))
		orb=strtrim(orbtemp,1)
		if orbtemp LT 1000 then $
patheph=strmid(path,0,strlen(path)-strlen(orb)-3)+'\eph\'
if orbtemp GT 1000 then $
patheph=strmid(path,0,strlen(path)-strlen(orb)-2)+'\eph\'
		...
		cd,patheph
		eph_path=patheph
		idl_path=strmid(path,0,strlen(path)-strlen(orb)-3)
		;stop
		;stop
 ;Check crfile.shr file for ephemerius file name corresponding to this orbit
		val_crfiles,orb,orb_binfile,orb_date
 ;Opens ephemeruis file and inputs parameters values
		cd,eph_path;,strmid(path,0,strlen(path)-5)
		;stop
		fileph,string(orb_binfile),ephv,path,state
...
if fil_info EQ 'SPECTRAL DOMAIN' then $
	begin
        ...
        dpphetaSB,cm_eph,cm_val,state,Dat5 $
		else $
		if dat5 EQ 'Ellipticity' THEN $
        Ellip,cm_eph,cm_val,state,Dat5 $
		else $
		if dat5 EQ 'VG' THEN $
        ...
        ELSE $
		DPOWN3,cm_eph,cm_val,state,Dat5
		;DPOWN3Pc5,cm_eph,cm_val,state,Dat5
...
;		mouseover.pro: Activates mouse to give values on plot (added 18/07/02).
pro filter_event,ev
common orbinfo,orb,orb_binfile,orb_date
common cm_crres,state,cm_eph,cm_val
;**************************************************
...
if Pros5 EQ 'Pc5 B' then $
begin
lowapc,Pros5
end

if Pros5 EQ 'Pc5 E' then $
begin
pc5Etest,Pros5
end

;****************************************************
if Pros5 EQ 'Mouse over' then $
begin
Mouseover
;end
end
...
;******************************************************************************************
;****************************************************
if Pros5 EQ 'Static Power' then $
begin
;Print,'hello'
static_power6;,cm_eph,cm_val,state
end
end
...
Ephermius_menu=WIDGET_BUTTON(bar,uvalue='ephm',VALUE='Ephemeris', /MENU)
...
;*****************************************************************************
;FFTSL  = Widget_Slider(base,value=300,UValue='FSL',$
;          Minimum=10,Maximum=1000,Title='FFT Length [Points]')
;draw = WIDGET_DRAW(base3,YSIZE=300,XSIZE=350,uvalue='drawID', /FRAME)
;dom2_menu =  CW_FIELD(base2,XSIZE=10, TITLE='Enter the Maximum Frequency Required (mHz):',uvalue='f8')
;dom3_menu =  CW_FIELD(base2,XSIZE=10, TITLE='Enter the FFT Analysis Length:',uvalue='f9')
;dom4_menu =  CW_FIELD(base2,XSIZE=10, TITLE='Enter the Time Resolution (Points):',uvalue='f9')

;dom5_menu =  CW_FIELD(base2,XSIZE=10, TITLE='Enter the Spectral Weighting, n [f^n]:',uvalue='f11')
;*****************************************************************************
eeph=strarr(1)
...
;WIDGET_CONTROL, base2,update=1
;WIDGET_CONTROL, base3,update=1
WIDGET_CONTROL, base,set_uvalue=state
WIDGET_CONTROL, state.eph_menu, sensitive=1
WIDGET_CONTROL, state.Dat_menu, sensitive=1
WIDGET_CONTROL, state.ser_menu, sensitive=1
WIDGET_CONTROL, state.text,set_value=string('Sun Aug 18 20:28:35 2002'),/append
WIDGET_CONTROL, state.text,set_value=string('IDL Codes Directory: c:\paul\phd\crres\eph_test'),/append
WIDGET_CONTROL, state.text,set_value=string('Telemetry Directory: c:\paul\phd\crres\data'),/append
WIDGET_CONTROL, state.text,set_value=string('Ephemeris Directory: c:\paul\phd\crres\eph'),/append
WIDGET_CONTROL, state.text,set_value=string('Results Directory: c:\paul\phd\crres\results'),/append
WIDGET_CONTROL, state.text,set_value=string('Events Paramters Directory: c:\paul\phd\crres\events'),/append
WIDGET_CONTROL, state.text,set_value=string('Logs Directory: c:\paul\phd\crres\log'),/append
WIDGET_CONTROL, state.text,set_value=string('Spline Sigma: 0.1'),/append
WIDGET_CONTROL, state.text,set_value=string('Frequency to Remove Spacecraft Phasing: 6.0Hz'),/append
WIDGET_CONTROL, state.text,set_value=string('Despike Limit (Points): 40.0'),/append
WIDGET_CONTROL, state.text,set_value=string('Despike Limit (Normalized Gradient): 1.0'),/append
WIDGET_CONTROL, state.text,set_value=string('Number Data Files: 3'),/append
WIDGET_CONTROL, state.text,set_value=string('orb1062_2030_2100a.val'),/append
WIDGET_CONTROL, state.text,set_value=string('orb1051_0830_0900a.val'),/append
WIDGET_CONTROL, state.text,set_value=string('orb1043_1800_2000a.val'),/append

WIDGET_CONTROL, state.text,set_value=string('Beginning main data processing...'),/append
WIDGET_CONTROL, state.text,set_value=string('Data File: 1'),/append
WIDGET_CONTROL, state.text,set_value=string('File: Orb1043_ 1800_ 2000a.val'),/append
WIDGET_CONTROL, state.text,set_value=string('Ephemeris File: c9127415'),/append
WIDGET_CONTROL, state.text,set_value=string('Extracting ephemeris information'),/append

...
; Main Program
;
Pro dpcrosspower_again_Z,DDat5
...
Begin
 ...
 For i=0,NFr-1 do $
	Begin
  		V1=(abs(CP[i]));^2.
  		V2=Float(FFTN)
  		SpArr(i)=V1/(V2*V2) ;Units MicroWatts or nT^2/Hz
 end
...
;********************************************************
 ...
 For i=0,FFTN-1 do TsArr(0,i)=data4(Posn+i)
 For i=0,FFTN-1 do TsArr(1,i)=data5(Posn+i)
End    ; end Bloc Loop
;
print,PnTRes
...
; stop
 ...
 Ttle[0]='Orbit'+orb+' '+orb_date+' BXY '+Dat5+' Spectrum'
 XTtle='Time (UT)'
 YTtle='Frequency (Hz)'
 YRngL=0
 Scl=1
 ;!P.multi=0
 DYNTV_crres1,CPArr,Title=Ttle[0],YTitle=YTtle,$
 XRange=[Min(ttt[0]),Max(ttt[Npnts-1])],YRange=[YRngL,MxF],$
 Scale=Scl,Range=[MnPow_Z[0],MxPow_Z[0]],Aspect=5.0,dat5=dat5,$
 Px1=Pxx1[0],Px2=Pxx2[0],Py1=Pyy1[0],Py2=Pyy2[0]

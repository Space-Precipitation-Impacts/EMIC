...
Function X000TLab,Value			;Function to format x axis into hours:minutes:seconds.frac
 mSec=long(Value)
...
Pro DigiPic,XRngL,XRngH,Nx,YRngL,YRngH,Ny,OX,OY,$
 TArr,FRArr,T15,Fr15,Pnts
common Poyntpow, PoyntArr
common pp,pxx,pyy,xs,ys
common interarr, aa
TArr=DblArr(1000)
FrArr=DblArr(1000)
...
; Print,!err
 ...
 Begin
  ...
  Dmy=float(y-YRngL)/Dm2
  ArrP=Fix(Dm*(Nx-1))
 YarrP=fix(Dmy*(Ny-1))
  If ArrP LT 0 Then ArrP=0
  If ArrP GT Nx-1 Then ArrP=Nx-1
  If YArrP LT 0 Then YArrP=0
  If YArrP GT Ny-1 Then YArrP=Ny-1
 ...
 ; FRArr(ii)=y
 ; ii=ii+1
  Lb=TLab+' UT '+StrTrim(String(y),2)+' Hz ';+strtrim(string(Poyntarr(ArrP,YarrP)))+' uW/m!U2!n'
  color=!d.n_colors-1

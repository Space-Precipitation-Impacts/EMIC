...
; Main Program
;
Pro group_velocity,cm_eph,cm_val,state,Dat5
...
;stop
 ;TrArr(2,*)=FFT(TsArr(2,*),1)
 Velg=dblarr(n_elements(TrArr(1,*)))
 ;CP=fltarr(n_elements(TrArr(1,*)))
;stop
 for yy=long(0), n_elements(TrArr(1,*)) -long(1) do $
 	begin
 		if TrArr(1,yy) EQ 0.0 then $
 		Velg[yy]=0.0 else $
 		Velg[yy]=[TrArr(0,yy)/(TrArr(1,yy))]
...
Set_Plot,'WIN'
 PAgn=0
 Print,'Minimum velocity is ',Min(DispArr)
 Print,'Maximum velocity is ',Max(DispArr)
 Erase
 Device,Decomposed=0
   ;13
  LoadCT,17
 ...
 tvlct,r,g,b
;ind=where(Disparr LT 10e-6,cou)
;for ii= 0, cou-1 do $
;Disparr[ind[ii]]=0.0

 Ttle='Orbit'+orb+' '+orb_date+' '+Dat5+' Spectrum'
 XTtle='Time (UT)'
 YTtle='Frequency (Hz)'
 YRngL=0
 Scl=1
 ...
 Scale=Scl,Range=[-200,200],Aspect=1.5,ymargin=10.5,dat5=dat5
 ;Arr=DispArr

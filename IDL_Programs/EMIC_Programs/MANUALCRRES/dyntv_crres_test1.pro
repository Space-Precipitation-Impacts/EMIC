...
;begin main
;
pro dyntv_crres_test1,a,scale=scale,range=range,xrange=xrange,yrange=yrange,aspect=aspect,$
           title=title,xtitle=xtitle,ytitle=ytitle,noframe=noframe,ymargin=ymargin,$
           xmargin=xmargin,dat5=dat5,cbtle=cbtle
            common namm,nn,ttt
...
if keyword_set(noframe) then begin
  ...
  plot,[0,0],[0,0],xstyle=1,ystyle=1,title=title,xtitle=xtitle,ytitle=ytitle, $
       ...
       /noerase,/device,/nodata,color=max_color,xticks=3
  endif else begin
  ...
  endelse
endelse
XYOUTS,pos[0]+px[1]+10,pos[1]+py[1]/5.,cbtle,orientation=90,/device
end

...
; Main Program
;
Pro DPpoynt2,cm_eph,cm_val,state,Dat5
...
;
;stop
SpArr=DblArr(NFr+2*ism)   ; Double Prec. Spectral Array
...
;********************************************************
 ...
 Begin
  ...
  For i=NFr-1-2*ism,NFr-1 do DispArr(Bloc,i)=SpArr(i)*Wght(i)
  For i=NFr-1-2*ism,NFr-1 do CPArr(Bloc,i)=abs(CP(i))*Wght(i)

  For i=iLFr,NFr-1-2*ism do $
  Begin
   DispArr(Bloc,i)=0.0
   CPArr(Bloc,i)=0.0
   Js=i-2*ism
...
MnPow=double(MnRng)
MxPow=double(MxRng)
MnCPow=double(MnCPRng)
MxCPow=double(MxCPRng)
;eph_inter_win,cm_eph,cm_val,state,Dat5
spectralpoyntps2,cm_eph,cm_val,state,Dat5,DispArr,CScl
...
; xx=congrid(cm_val.(0),40)
;ps_contour, arg1,xx,Freqarr,/PS,ctab=13,xtickformat='XTLab'
;DPChoice_poynt,Dat5
Print,'Finished'
;stop
end

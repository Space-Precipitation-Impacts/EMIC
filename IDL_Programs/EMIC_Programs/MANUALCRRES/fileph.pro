...
if (error NE 0) then $
begin
Result = DIALOG_MESSAGE('No ephmerius file found in default directory...will try d: drive?')
cd,'d:\'
pathtemp='d:\'
openr,/get_lun,u,fnm,error=error
if (error NE 0) then $
;begin
Result = DIALOG_MESSAGE('No ephmerius file found!')
;exit(1)
;end
end
filss=fnm
...
begin
 ...
 if strmid(pathtemp,strlen(path)-4,4) EQ 'eph' then $
 cd,strmid(pathtemp,0,strlen(path)-4) else $
 if strmid(pathtemp,strlen(path)-5,4) EQ 'data' then $
 cd,strmid(pathtemp,0,strlen(path)-5) else $
 cd,strmid(pathtemp,0,strlen(path)-9)

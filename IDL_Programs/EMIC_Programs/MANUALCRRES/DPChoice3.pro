...
Case UVal of
 ...
 'MFS' :Begin
 		Widget_Control,ev.id,Get_Value=MxF
 		end
 'PRS' : Begin
...
Dat5a=Dat5
;stop
base5=widget_base(/column,xsize=300,title='Spectral Widget',/ALIGN_TOP)
;FFTN=600
;stop
...
;stop
 ;MFSL   = widget_Slider(base5,value=MxF,Format='(F2.1)',UValue='MFS',Maximum=NyqF,$
 ;          Title='Max. Freq. [Hz] Nyquist = '+StrTrim(String(NyqF),2)+' Hz')
MFSL   = cw_Fslider(base5,value=MxF,Format='(F5.3)',UValue='MFS',Maximum=5.0,$
           Title='Max. Freq. [Hz] Nyquist = '+StrTrim(String(NyqF),2)+' Hz',/drag,/frame,xsize=250)
;
;MFSL   = CW_Field(base5,uvalue=MxF,$
;           Title='Max. Freq. [Hz] Nyquist = '+StrTrim(String(NyqF),2)+' Hz',/string)

...
count=0L
cd,path+'density\'
Openr,uden,'orb1029a.den',/get_lun
text=''
readf,uden,text
readf,uden,text
...
if fix(strmid(eltim(i),0,2)) GE 2 and fix(strmid(eltim(i),0,2)) LT 3 then $
begin
		addtim='24:00:00'
...
;for i=0,n_elements(eltim)-1 do $
eltimlon[i]=strtrim(XXTLab2(eltim[i]))
;stop
ttt=cm_val.(0)
UT=eltimlon
;
...
device,filename='c:\paul\phd\crres\orb1029_aZ2.ps',yoffset=3, ysize=25,/portrait

!P.charsize=2.0

plot,cm_val.(0),val_inter_dens,xstyle=5,xtickformat='XTLab',title='Orbit1029 Densities',$
ytitle='Number Density (cm!U3!n)',ymargin=[-6,7];,xtitle='UT'
...
phase_vel= -smooth(cm_val.(2)/cm_val.(4),40000,/edge_truncate)*1000.;vel_c/([1.+sum_hyd+sum_hel]^(0.5))
;stop


;*************************************************************************

plot,cm_val.(0),cm_val.(4),xstyle=5,ytitle='dBx (nT)',xtickformat='XTLab',$
yrange=[-10,10],YSTYLE=1,xticks=3,ymargin=[-5,7];,title='Velocities';,XMARGIN=1.0
plot,cm_val.(0),cm_val.(2),xstyle=5,ytitle='Ey (mV/m)',xtickformat='XTLab',$
yrange=[-4,4],YSTYLE=1,xticks=3,ymargin=[-5,7]

plot,cm_val.(0),alf_vel/1000.,xstyle=9,ytitle='Va & Vp (km/s)',xtickformat='XTLab',$
yrange=[-1500.,1500.],YSTYLE=1,xticks=3,title='Velocities',ymargin=[-10.3,8];,/ylog;,XMARGIN=1.0

oplot,cm_val.(0),phase_vel,linestyle=2.0;,xstyle=9,ytitle='Va (km/s)',xtickformat='XTLab',title='Alfven Velocity',
;,xrange=[cm_val.(0)[0],max(cm_val.(0))],YSTYLE=1,xticks=3;,XMARGIN=1.0
!P.charsize=2.0
...
;*************************************************************************
endelse
;densityps2,cm_eph,cm_val,state,Dat5
end
...
for i=0,ct-1 do $
          begin
             ...
             data_32[i] = 3.0*sin(2.*Pi*20.*i/ct)
             data_32a[i] =3.0*sin(2.*Pi*20.*i/ct+ Pi/1.5);+(Pi/2.0))
endfor
count=long(0)
;stop
!P.Multi = [0,1,2]
...
Window,0, Xsize=700,Ysize=600,title='Test Pattern Wave field'
    plot,tims_32,data_32*data_32a,XStyle=1,Xticks =3,XTickFormat='XTLabbb',xtitle ='Time (UT)',$
    ytitle='nT!U2!n',$
...
print,Xdat1[100],'  ',tim(tims_32[100])

	DXMAX=MAX(abs(Xdat1*Xdat2));
;stop
StaL='CRRES ORBIT'
Year=2001
Month=10
...
ism=0
Sum1=0.0
For i=0,FFTN-1 do TsArr1(i)=XDat1(i)
For i=0,FFTN-1 do TsArr2(i)=XDat2(i)
If (WndT EQ 2) Then $
Begin
...
;*********************************************************************
 Posn=(Bloc+1)*TRes
 For j=0,FFTN-1 do TsArr1(j)=XDat1(Posn+j) ; New Data
For j=0,FFTN-1 do TsArr2(j)=XDat2(Posn+j)
End    ; end Bloc Loop

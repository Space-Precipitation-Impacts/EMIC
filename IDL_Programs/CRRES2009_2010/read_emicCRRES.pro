...
;template folder in order to get the yearday associated with the orbit out.
  ...
  restore, Templatefolder+'CRRES_emic_template.save', /ver
  ...
  CRRESsave =  strcompress(datafolder+'CRRES_EMIC'+'.save',$
               /remove_all)
  CRRESfile = strcompress(datafolder+'CRRES_emic.txt',$
                          /remove_all)  
  ...
  for i = 0l, n_elements(orbit)-1 do begin
     ...
     CRRES_jul90 = julday(0,doy(index90), 1990, ut(index90),0,0) 
     CRRES_jul91 = julday(0,doy(index91), 1991, ut(index91),0,0) 
     ...
     emicyear = year
     save, orbit, ut, endut, doy, duration, year,  emicyear, crres_jul90, crres_jul91, crres_event90, crres_event91, julemicCRRES_90, julemicCRRES_91, startCRRES_90, startCRRES_91, filename = CRRESsave

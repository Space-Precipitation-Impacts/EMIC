pro norm_sym ;, datafolder, savefolder, tempfolder, figurefolder, onsetlength, mainlength, recovlength, opcr, mpcr, rpcr
...
;everything had to be made sure to be double persision. 
  for l = 0, n_elements(day)-1 do begin
     jday = julday(month(l), day(l), year(l), hour(l), omm, 0.d)
     onset_time(l) = jday ;jul2yday(jday) - year*1000.
     mjul(l) = julday(mmonth(l), mday(l), year(l), mhour(l), mmm, 0.d)
     ejul(l) = julday(emonth(l), eday(l), year(l), ehour(l), emm, 0.d)
  endfor
...
;fostartcr = congrid(ostartcr, onsetlength) ;this isn't quiet right but we'll use for now to get an idea.
     ...
     for nn = 0, n_elements(all_normsym(0,*))-1 do begin 
        ...
        plot, x, prec50, ytitle = 'Sym (nT) and occurance of EMIC waves (counts*10.) ', $
              ...
              xtitle = 'phase of storm', xrange = [min(x), max(x)], xstyle = 1   ;, ymargin = [0.1,2] ;, psym = 5
...
;        write_PNG, filename1, image, r, g, b
        ...
        plot, ox, onsetmean, ytitle = 'Sym (nT) and occurance of EMIC waves ', $
              yrange = [-150, ymax], ystyle = 1, $ ;yrange = [min(all_normsym), max(all_normsym)], $ 
...
;        oplot, rx, histogram(rallstartcr, binsize = floor(.05*recovlength))*10, color = 200
        ...
        plot, ox, make_array(n_elements(ox), value = 0), ytitle = 'Sym (nT) ', $
              ...
              xtitle = '3 hours before onset', ymargin = [0.1,.01],pos = [0.,0.,.15,.15] ;, psym = 5
        ...
        loadct, 0
        plot, ox, ohistcr, ytitle = 'Occurance of EMIC waves', yrange = [0., max([ohistcr, mhistcr, rhistcr])], ystyle = 1, pos = [0.,.15,.15,1.] ;, psym = 5


        mx = findgen(mainlength)/mainlength *100.
        plot, mx, make_array(n_elements(mx), value = 0), $
              yrange = [0, 4], ystyle = 1, xrange = [0,n_elements(mx)], xstyle = 1,$
        ...
        loadct, 0
        plot, mx, mhistcr, pos = [.15,.15,.5,1.],  title = 'Normalized Storms for the CRRES mission and EMIC wave occurance', yrange = [0., max([ohistcr, mhistcr, rhistcr])], ystyle = 1

        rx = findgen(recovlength)/recovlength *100.
        plot, rx, make_array(n_elements(rx), value = 0), $
              yrange = [0, 4], ystyle = 1, xrange = [0,n_elements(rx)], xstyle = 1,$
        ...
        loadct, 0
        plot, rx, rhistcr, ymargin = [.01,.01],pos = [.5,.15,.90,1.0], yrange = [0., max([ohistcr, mhistcr, rhistcr])], ystyle = 1 ;, psym = 5
        ...
        plot,findgen(10),ycb, pos = [.95,0.,1.,1.],$
...
;        print, numbcolors, ' colors in plot', minoff, ' offset to Sym'
        ...
        plot, mx(0:n_elements(mx)-1), make_array(n_elements(mx), value = 0), $
        ...
        plot, rx(0:n_elements(rx)-1), make_array(n_elements(rx), value = 0), $
              yrange = [0, numstorms], ystyle = 1, xrange = [0,100.], xstyle = 1,$

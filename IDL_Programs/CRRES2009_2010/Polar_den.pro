...
;  endfor
                                ...
                                ;Here we read in the Density data from CRRES
  restore, '../Data/CRRES_1min_1990.save'
  restore, '../Data/CRRES_1min_1991.save'

  bad90 = where(den90 lt 0.) 
  bad91 = where(den91 lt 0.) 
  den90(bad90) = 0.
  den91(bad91) = 0.
  all_den90 = den90
  all_den91 = den91
...
;  numarray = [oncrres90 * storm90,oncrres91 * storm91]
  den90 = all_den90*storm90
  den91 = all_den90*storm91
...
;  numonarray = [oncrres90 * onphase90,oncrres91 * onphase91]
  oden90 = all_den90*onphase90
  oden91 = all_den91*onphase91
...
;  nummainarray = [oncrres90 * mainphase90,oncrres91 * mainphase91]
  mden90 = all_den90*mainphase90
  mden91 = all_den91*mainphase91
...
;  numrecovarray = [oncrres90 * recovphase90,oncrres91 * recovphase91]
  rden90 = all_den90*recovphase90
  rden91 = all_den91*recovphase91
  rsym90 = sym90*recovphase90
  rsym91 = sym91*recovphase91


  maxden90 =  max(all_den90(where(finite(all_den90))))
  maxden91 =  max(all_den91(where(finite(all_den91))))
  maxdensity = max([maxden90, maxden91])*10.
;********************************************************************************************************************
;********************************************************************************************************************
...
;wave events.
        ...
        mls = [mls90, mls91]
        rls = [rls90, rls91]
        tden  = [all_den90, all_den91] ;t for total
        tmlt = [hmlt90, hmlt91]
        tls = [hls90, hls91]

        for j = 3., n_elements(tpolarden(0,*))-1 do begin
           ...
           for i = 0, n_elements(tpolarden(*,0))-1 do begin
              ...
              if use(0) ne -1 then begin
                 tpolarden(i,j) = mean(tden(use),  /nan)
              endif else begin
                 tpolarden(i,j) = 0.
              endelse
           endfor                      
...
;wave events.
        for j = 3., n_elements(polarden(0,*))-1 do begin
...
;events occurring during the onset phase        
        ...
        for j = 3., n_elements(opolarden(0,*))-1 do begin
...
;                 print, mean(oden(ouse),  /nan)
                 opolarden(i,j) = mean(oden(ouse),  /nan)
              endif else begin
                 opolarden(i,j) = 0.
              endelse
           endfor                      
...
;events occurring during the main phase        
        ...
        for j = 3., n_elements(mpolarden(0,*))-1 do begin
...
;                 print, mean(mden(muse), /nan)
                 mpolarden(i,j) = mean(mden(muse), /nan)
              endif else begin
                 mpolarden(i,j) = 0.
              endelse
           endfor                      
...
;events occuring during the storm phase.
        ...
        for j = 3., n_elements(rpolarden(0,*))-1 do begin
...
;                 print, mean(rden(ruse), /nan)
                 rpolarden(i,j) = mean(rden(ruse), /nan)
              endif else begin
                 rpolarden(i,j) = 0.
              endelse
           endfor                      
        ...
        qden  = [all_den90*quiet90, all_den91*quiet91] ;q for quiet
        qmlt = [hmlt90*quiet90, hmlt91*quiet91]
        qls = [hls90*quiet90, hls91*quiet91]

        sden  = [all_den90*storm90, all_den91*storm91] ;s for storm
        ...
        for j = 3., n_elements(qpolarden(0,*))-1 do begin
           ...
           for i = 0, n_elements(qpolarden(*,0))-1 do begin
              ...
              use = where(meep eq 1)
              if use(0) ne -1 then begin
                 qpolarden(i,j) = mean(qden(use), /nan)
              endif else begin
                 qpolarden(i,j) = 0.
              endelse
           endfor                      
...
;wave events.
        ...
        for j = 3., n_elements(spolarden(0,*))-1 do begin
           ...
           for i = 0, n_elements(spolarden(*,0))-1 do begin
              ...
              use = where(meep eq 1)
              if use(0) ne -1 then begin
                 spolarden(i,j) = mean(sden(use), /nan)
              endif else begin
                 spolarden(i,j) = 0.
              endelse
           endfor                      
        ...
        ;denrange = maxdensity ;
        denrange = max([polarden, opolarden, mpolarden, rpolarden, tpolarden, spolarden, qpolarden])*1.3
        ...
        !P.multi=[0,3,3]
        set_plot, 'PS'
        device, filename = '../figures/den_polar.ps', /landscape, /color
...
;        xyouts, -10.1, 0.5, '9 RE', charsize = ch
        ...
        loadct, 0
        ;denrange = maxdensity
                                ;max([all_den90, all_den91]) ; max([polarden, opolarden, mpolarden, rpolarden, tpolarden]) + 1.; max(polarden) 
        print, 'max average density ', denrange
        print, 'max range', max([polarden, opolarden, mpolarden, rpolarden, tpolarden, spolarden, qpolarden])
